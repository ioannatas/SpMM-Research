...
compute(char * matrix_name,
		...
		struct Mask * Mask,
		csr_matrix * AM,
		ValueType * x, double * x_ref,double * z_ref,ValueType * y,
		long min_num_loops,
		long print_labels)
{
	...
	if (!print_labels)
	{
		...
		if (MF->format_name=="MKL_GEMM"){
			...
			gflops = Mask->nnz * 2 * 1e-9 * n / time * num_loops ;    // Use csr_nnz to be sure we have the initial nnz (there is no coo for artificial AM).
			// printf("wright %d * %d * %d / %lf * %ld * 2 * 1e-9;\n", csr_k, csr_m , n, time ,num_loops);
		}
	}
...
main(int argc, char **argv)
{
	...
	if (argc == 1)
	{
		compute(NULL, NULL, NULL, NULL, NULL, 0, 0, 0, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, 0, 1);
		return 0;
	}
...
child_proc_label:
	...
	for( n=num_cols ; n <num_cols + 1; n*=4){
		x_ref = (typeof(x_ref)) aligned_alloc(64, csr_m * n * sizeof(*x_ref));
		x = (typeof(x)) aligned_alloc(64, csr_m * n * sizeof(*x));
		z_ref = (typeof(z_ref)) aligned_alloc(64, csr_m * n * sizeof(*z_ref));
		z = (typeof(z)) aligned_alloc(64, csr_m * n * sizeof(*z));
		mask= create_mask(sparse_attention_type,sddmm_sparsification_type, csr_m, sparsity, l_sparsity, band_size);
		#pragma omp parallel for
