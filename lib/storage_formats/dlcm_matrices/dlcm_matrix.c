...
#include "dlcm_matrix.h"

#undef  TYPE
#undef  SUFFIX
#define TYPE  MATRIX_MARKET_FLOAT_T
#define SUFFIX  _f
#include "dlcm_matrix_gen.c"
...
smtx_functor_get_value(struct DLCM_Matrix * MTX)
{
...
void
smtx_init(struct DLCM_Matrix * MTX)
{
	MTX->filename = NULL;
	MTX->format = NULL;
	MTX->field = NULL;
...
void
smtx_clean(struct DLCM_Matrix * MTX)
{
	if (MTX == NULL)
		return;
	free(MTX->filename);
...
void
smtx_destroy(struct DLCM_Matrix ** MTX_ptr)
{
	if (MTX_ptr == NULL)
		return;
	smtx_clean(*MTX_ptr);
...
static inline
long
smtx_parse_header(struct File_Atoms * A, struct DLCM_Matrix * MTX)
{
	char ** lines;
	long i = 0, j;
	int num_chars;
	...
	if (sscanf(lines[i], "%ld,%ld,%ld", &M, &K, &nnz_sym) != 3)
			error("Error parsing DLCM matrix '%s': invalid/missing matrix sizes: %s\n", MTX->filename, lines[i]);
...
{
	struct DLCM_Matrix * MTX = malloc(sizeof(*MTX));
	struct File_Atoms * A;
	char ** lines;
	long * lengths;
	long i;
...
static
void
smtx_plot_base(struct DLCM_Matrix * MTX, char * filename, int plot_density)
{
	struct Figure * fig;
...
void
smtx_plot(struct DLCM_Matrix * MTX, char * filename)
{
...
void
smtx_plot_density(struct DLCM_Matrix * MTX, char * filename)
{
